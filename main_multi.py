...
# We follow original code base from Hexplane link 
...
def render_test(cfg):
    ...
    if cfg.render_train:
        os.makedirs(f"{logfolder}/imgs_train_all_mic", exist_ok=True)
        train_dataset = get_train_dataset(cfg, is_stack=True)
     
        evaluation(
            train_dataset,
            Cplane,
            gen_latents_lists_new[1],
            cfg,
            f"{logfolder}/imgs_train_all_mic/",
            prefix="train",
            N_vis=-1,
            N_samples=-1,
            ndc_ray=ndc_ray,
    ...
    if cfg.render_test:
        os.makedirs(f"{logfolder}/imgs_test_mic", exist_ok=True)
        evaluation(
            test_dataset,
            Cplane,
            gen_latents_lists_new[2],
            cfg,
...
def reconstruction(cfg):
    if cfg.data.datasampler_type == "rays":
        train_dataset1 = get_train_dataset(cfg,name="none", is_stack=False)
        cfg.data.datadir = "./nerf/nerf_synthetic/nerf_synthetic/hotdog"
        train_dataset2 = get_train_dataset(cfg,name="none", is_stack=False)
        cfg.data.datadir = "./nerf/nerf_synthetic/nerf_synthetic/mic"
        train_dataset3 = get_train_dataset(cfg,name="none", is_stack=False)
        cfg.data.datadir = "./nerf/nerf_synthetic/nerf_synthetic/chair"
        train_dataset4 = get_train_dataset(cfg,name="none", is_stack=False)
        cfg.data.datadir = "./nerf/nerf_synthetic/nerf_synthetic/lego"
        default = get_train_dataset(cfg,name="none", is_stack=False)
    ...
    if cfg.data.dataset_name == "dnerf":
        ...
        class1_raysize = range(train_dataset1.all_rays.shape[1])
        class2_raysize = range(train_dataset2.all_rays.shape[1])
        class3_raysize = range(train_dataset3.all_rays.shape[1])
        class4_raysize = range(train_dataset4.all_rays.shape[1])

        rank1 = train_dataset1.all_rays.shape[1]
        rank2 = train_dataset2.all_rays.shape[1]
        rank3 = train_dataset3.all_rays.shape[1]
     
        class_mapping1 = dict([ (name, [i, 0]) for i, name in enumerate(class1_raysize)] )
        class_mapping2 = dict([ (name+rank1, [rank1+i, 1]) for i, name in enumerate(class2_raysize) ])
        class_mapping3 = dict([ (name+rank1+rank2, [rank1+rank2+i, 2]) for i, name in enumerate(class3_raysize) ])
        class_mapping4 = dict([ (name+rank1+rank2+rank3, [rank1+rank2+rank3+i, 3]) for i, name in enumerate(class4_raysize) ])
    ...
    class_mapping = merge(class_mapping, class_mapping3)
    class_mapping = merge(class_mapping, class_mapping4)
    ...
    if cfg.render_test:
        os.makedirs(f"{logfolder}/imgs_test_all_lego", exist_ok=True)
        evaluation(
            test_dataset,
            Cplane,
            test_latents[0],
            cfg,
            f"{logfolder}/{cfg.expname}/imgs_test_all_lego/",
            prefix="test",
            N_vis=-1,
            N_samples=-1,
            ndc_ray=ndc_ray,
    ...
    if cfg.render_path:
        os.makedirs(f"{logfolder}/{cfg.expname}/imgs_path_all_mic/", exist_ok=True)
        evaluation_path(
            test_dataset,
            Cplane,
            gen_latents_lists_a[2],
            cfg,
            f"{logfolder}/{cfg.expname}/imgs_path_all_mic/",
            prefix="validation",
            N_vis=-1,
            N_samples=-1,
            ndc_ray=ndc_ray,
