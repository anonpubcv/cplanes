...
from cplane.render.util.metric import rgb_lpips, rgb_ssim
...
def OctreeRender_trilinear_fast(
    ...
    for chunk_idx in range(N_rays_all // chunk + int(N_rays_all % chunk > 0)):
        ...
        gen_latents_chunk = gen_latents[chunk_idx * chunk : (chunk_idx + 1) * chunk].to(device)
...
def evaluation(
    ...
    gen_latents =np.array_split(gen_latents1, 40)
    ...
    for idx in tqdm(idxs):
        ...
        sample_times = torch.add(sample_times,0)
        depth = None
        ...
        gen_latent = gen_latents1[None,None, :].expand(
        (rays.shape[0], 1, gen_latents1.shape[-1])
        )
...
def evaluation_path(
    ...
    for idx in tqdm(range(val_times.shape[0])):
        ...
        gen_latent=gen_latents
        gen_latent = gen_latent.view(-1, gen_latent.shape[-1])
        gen_latent = gen_latent[:, None].expand(
        (rays.shape[0], 1, gen_latent.shape[-1])
        )
        rgb_map, _, depth_map, _, _ = OctreeRender_trilinear_fast(
            rays,
            time,
