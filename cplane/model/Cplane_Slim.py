...
class Cplane_Slim(Cplane_Base):
    """
    A simplified version of Cplane, which assumes the following:
    1. Fusion-One is "Multiply" and Fusion-Two is "Concat".
    2. We directly calculate densities from Cplane without MLPs, which means  DensityMode is "plain" and density_dim = 1.
    """
    ...
    def __init__(self, aabb, gridSize, device, time_grid, near_far,gen_latents_lists, **kargs):
        ...
        )  # Assume we directly calculate densities from Cplane without MLPs.
    ...
    def compute_densityfeature(
        ...
        for idx_plane in range(len(self.density_plane)):
            ...
            gen_feat = F.grid_sample(
                    self.density_line_time[idx_plane],
                    line_gen_coord[[idx_plane]],
                    align_corners=self.align_corners,
                ).view(-1, *xyz_sampled.shape[:1])
            
            gen_space_feat =  F.grid_sample(
                    self.density_line_time[idx_plane],
            ...
            density_feature = density_feature + torch.sum(
                gen_space_feat*gen_feat*plane_feat * line_time_feat * self.density_basis_mat[idx_plane], dim=0
            )
    ...
    def compute_appfeature(
        ...
        plane_feat, line_time_feat,gen_feat,gen_space_feat  = torch.cat(plane_feat), torch.cat(line_time_feat), torch.cat(gen_feat),torch.cat(gen_space_feat)

        return self.app_basis_mat((plane_feat * line_time_feat*gen_feat*gen_space_feat).T)
